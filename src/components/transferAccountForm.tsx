...
const TransferAccountForm = () => {
    ...
    register("accountId", { required: "Este campo es requerido", min: { value: 1, message: "Seleccione una cuenta válida" } });
    ...
    const onSubmit = (data: {
        ...
        if (transfer.step === TransferSteps.Step1) {
            dispatch(transferSlice.actions.addAccountFrom({
        ...
        if (transfer.step === TransferSteps.Step2) {
            dispatch(transferSlice.actions.addAccountTo({
    ...
    }, [transfer.step, reset, transfer.accountFrom, transfer.accountFromType, transfer.accountTo, transfer.accountToType]);
