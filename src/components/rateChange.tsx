...
const RateChange = () => {
    ...
    const [fromRate, toRate] = useMemo(() => {
        const findRate = (a?: string, b?: string) => rateChanges.find((r) => r.fromCurrency === a && r.toCurrency === b);
        const fmt = (r?: { fromCurrency: string; rate: number }) => (r ? `${r.fromCurrency}: ${r.rate}` : undefined);
